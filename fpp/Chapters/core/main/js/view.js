  ...
  function initLabsMenu() { //---->
    ...
    p.labsData.chapters.forEach(function(chapter) {
      ...
      if (chapter.bottomGap) {
        $(createElement({
          tagName: "div"
        })).addClass("pLabBottomGap").appendTo(chapWrapper).css("padding-bottom", chapter.bottomGap);
      }
    });
...
//This function creates TOC.
 ...
 //This function updates the menu related to chapters.
  function loadTopic(topic_no) {
    ...
    $(p.mShell).find('.pTopicName').html("Ch " + (topic_no) + ": " + temp.title);
        ...
        if (!lessonFlag) {
          if (j == 0) {
            ...
            if (temp.unit[i]["section"][j].hasOwnProperty("hyperLink")) {
              ...
              if (temp.unit[i]["section"][j].hasOwnProperty("number")) {
                _pSubtopic.clone().appendTo(_pTopicBody).attr("data-chap", topic_no).attr("data-topic", i).attr("data-subtopic", j).html(temp.unit[i]["section"][j]["SectionTitle"])
              } else {
                _pSubtopic.clone().appendTo(_pTopicBody).attr("data-chap", topic_no).attr("data-topic", i).attr("data-subtopic", j).html((Number(i) + 1) + "." + _index + ": " + temp.unit[i]["section"][j]["SectionTitle"])
              }
            }
          }
        } else {
...
//This function is called when user click on topic in the inner menu
  function subMenuTopicDown(e) {
    ...
    if (!$(this).hasClass("sopen")) {
      $(this).find('.pSubTopicName .accSysmbol').not('.pSubTopicInnerWrap .accSysmbol').html("-");
  ...
  function loadJSActivity(ref) {
    var dimensions = JSActivityDimensions.hasOwnProperty(ref) ? JSActivityDimensions[ref] : {
      width: 760,
      height: 600
    }
...
// functions removes hover effect from the button
  ...
  function playerbtnManager(_obj) {
    ...
    switch (_obj.type) {
      ...
      case "volume":
        overlayDown()
        $(p.mShell).find(".pVolumeSlider").toggle("slide", {
          direction: "down"
        });
  ...
  function validatePassword() {
    ...
    if (!valFlag) {
      if (newPassword == confPassword && res && oldPass) {
        showLoader();
        $(document).trigger("changePassword", {
