var Lab = (function(){
	...
	Activity.prototype.startLab = function(){
		...
		this.layout.vSpeedStepper.addEventListener("valueChange", function(e){
			...
			if(scope.playerIndex == 0 && scope.activePlayer.arrow.angle == scope.activePlayer.arrow._maxAngle){
				v1.set(0, e.currentTarget._direction * e.currentTarget._step * -2);
			} else if(scope.playerIndex == 1 && scope.activePlayer.arrow.angle == scope.activePlayer.arrow._minAngle){
				v1.set(0, e.currentTarget._direction * e.currentTarget._step * 2);
			}
	...
	Activity.prototype.reset = function(){
		...
		this.layout.tower_blast_1.cache.apply(this.layout.tower_blast_1, this.cacheData.tower_blast);
		this.layout.tower_blast_1.removeAllEventListeners("tick");
		this.layout.tower_blast_2.gotoAndStop(0);
		this.layout.tower_blast_2.cache.apply(this.layout.tower_blast_2, this.cacheData.tower_blast);
		...
		this.player1/* .data("velocity", this.player1.data("defaultV")) */.data("point0", false).data("point1", false);
		this.player1/* .data("angle", this.player1.data("defaultA")) */.reset();
		this.player2/* .data("velocity", this.player2.data("defaultV")) */.data("point0", false).data("point1", false);
		this.player2/* .data("angle", this.player2.data("defaultA")) */.reset();
		this.player1.turn = 0;
		this.player2.turn = 0;
	...
	function fireClicked(e){
		...
		if(this.activePlayer.auto){
			...
			this.activePlayer.fire(true, this.layout.hSpeedStepper.value, this.layout.vSpeedStepper.value);
		}
	...
	function nextTurn(){
		...
		this.layout.hSpeedStepper.updateMinMax(this.activePlayer.data("minX"), this.activePlayer.data("maxX"), 0.1, this.activePlayer.data("X"));
	...
	function createPlayers(){
		...
		this.vLine1 = new VectorLine(this.layout.arrow1, {x:this.layout.cannon1.x, y:this.layout.cannon1.y}, {min:15, max:100}, {min:270, max:270+45}, this.layout.arrow1Line, {color:"#0066FF"}, 0.5);
		...
		}, {min:15, max:100}, {min:270-45, max:270}, this.layout.arrow2Line, {color:"#F22E1E"}, 0.5);
		...
		this.player1.data("velocity", this.player1.data("defaultV")).data("angle", this.player1.data("defaultA"));
		this.player2.data("velocity", this.player2.data("defaultV")).data("angle", this.player2.data("defaultA"));
		this.player1/* .reset() */.addEventListener("dataChange", this._onDataChange);
		this.player2/* .reset() */.addEventListener("dataChange", this._onDataChange);
	...
	function onDataChange(e){
		...
		if(this.activePlayer.arrow.y < -60 * this.activePlayer.arrow.unit){
			_p = Model.doIntersect({x:-1000, y:-60 * this.activePlayer.arrow.unit}, {x:1024, y:-60 * this.activePlayer.arrow.unit}, {x:0, y:0}, scope.activePlayer.arrow._vector);
			...
			_p = Model.doIntersect({x:-200, y:-35 * this.activePlayer.arrow.unit}, {x:1024, y:-35 * this.activePlayer.arrow.unit}, {x:0, y:0}, v);
		}
		if(_p){
			if(this.playerIndex == 0){
				_p = new Vector(Math.max(0, _p.x), _p.y);
		...
		var v = Math.abs(this.activePlayer.arrow.y / this.activePlayer.arrow.unit).toFixed(1) * 1;
		var h = (this.activePlayer.arrow.x / this.activePlayer.arrow.unit).toFixed(1) * 1;
...
Lab.data = {
	player1:{
		...
		boundary:[
			...
			[{x:1024,y:-100}, {x:1024,y:1000}],		// out of screen
		]
	},
	player2:{
		direction:-1,
		...
		boundary:[
			...
			[{x:0,y:-100}, {x:0,y:1000}],		// out of screen
		]
	}
}
