	...
	function NumericStepper(o){
		...
		if(o.horizontal){
			if(this._inc.getBounds().width){
				this._inc.x = this._dec.getBounds().width + (o.hasOwnProperty("gap") ? o.gap : 5);
			} else {
				this._inc.x = this._dec.children[0].image.naturalWidth + (o.hasOwnProperty("gap") ? o.gap : 5);
			}
	...
	Object.defineProperty(SpeedoMeter.prototype, "value", {
		...
		set:function(value){
			...
			if(value >= 0){
				...
				this._value = Math.max(this._minValue, Math.min(this._maxValue, Math.abs(value)));
				this.needle.rotation = this._minAngle - this._value * (this._maxAngle - this._minAngle) / (this._maxValue - this._minValue);
			}
...
var VectorLine = (function(){
	function VectorLine(bmp, startPoint, magnitude, angle, shape, object, _snapValue, restriction){
		this._bmp = bmp;
			...
			if(this.restriction){
				...
				if(p.length() <= (this._unit)*this.magnitude){
					...
					if(this._snapValue >= 1){
						...
						decimal = (this._snapValue < decimal) ? (decimal - (decimal % this._snapValue)) : 0;
						angle = round + decimal;
					}
