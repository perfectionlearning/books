...
var scope = this;
	...
	function play(){
		if(!this.init){
			...
			this.car1._initialize(c1.name,c1.maxAcceleration,c1.maxVelocity,c1.velocity,c1.velocityX,c1.displacementX,c1.displacementY,c1.angle,c1.track,c1.animation);
			this.car2._initialize(c2.name,c2.maxAcceleration,c2.maxVelocity,c2.velocity,c2.velocityX,c2.displacementX,c2.displacementY,c2.angle,c2.track,c2.animation);
			this.init=true;
...
var control=(function(){
	...
	function initialize(name,maxAcceleration,maxVelocity,velocity,velocityX,displacementX,displacementY,angle,track,animation){
	...
	function updateState(){
		...
		if(this.name=="redCar"){
			...
			lab.layout.line2.graphics.clear().setStrokeStyle(34).beginStroke("red").moveTo(0,0).lineTo(this.velocityX,0).endStroke();
			...
			if(this.movingStraight == true){
				if(this.position<3){
					if(this.velocityX < this.maxVelocity[this.position]){
						...
						if(((this.velocityX >= 0 || this.displacementX < (this.track[this.position].center.x-1)) && this.position == 1 && !this._checkSpeed)){
							this.velocityX=this.maxVelocity[this.position];
						}
					}
					else if(this.velocityX > this.maxVelocity[this.position]){
						this.velocityX =this.velocityX +(this.accelerationX *timeStep * 0.11);
						if((this.velocityX <= 0 || this.displacementX > (this.track[this.position].center.x-1)) && this.position == 2){
							this.velocityX=this.maxVelocity[this.position];
						}
					}
					else{
				...
				if(this.accelerating && this.velocityX >= this.maxVelocity[this.position]){
					this.accelerating=false;
				}
				if(this.name=="redCar"){
				...
				this.centripetalAcc = Math.abs( this.velocity.toFixed(1) * this.velocity.toFixed(1)/ this.track[this.position].radius);
				if(this.name=="redCar"){
	...
	function startConditions(){
		// if(this.maxVelocity[this.position] != 0){ 
			if(this.position == 0){
				this.accelerationX = (0.5 * (this.maxVelocity[this.position] * this.maxVelocity[this.position])/26.2);
			}
			else if(this.position == 1){
				this.vdiff=((this.maxVelocity[this.position] * this.maxVelocity[this.position]) - (this.maxVelocity[this.position-1] * this.maxVelocity[this.position-1]));
