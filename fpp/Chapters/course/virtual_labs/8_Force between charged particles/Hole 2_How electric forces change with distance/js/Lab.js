...
var ChargeClass = (function(){
	...
	Class.prototype._update = function(_charges, count){
		if(this.started){
			...
			if((lab.layout.ball.x < 904 && lab.layout.ball.x > 878 && lab.layout.ball.y<498 && lab.layout.ball.y>475) && (this._velocity.magnitude()<0.2)){
				this._Hole(_charges);
			}
			else{
	...
	Class.prototype._moveObject = function(_charges){
		...
		if(this.type == TYPE_BALL){
			...
			//deal with negatively charged putter.
 			if (_charges[0].charge < 0 && (Maths.getDistance(_charges[0], this) / this._unit) < 0.1/* (_charges[0].xRadius / this._unit) < (.1) */){
				this._velocity.set(0, 0);
	...
	Class.prototype._click=function(){
		...
		window.location.href="../Hole 3_How the charge values affect the force/index.html?"+a+"&"+b;
	}
