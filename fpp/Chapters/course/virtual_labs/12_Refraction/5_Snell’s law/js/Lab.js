var Lab = (function () {
	...
	function checkWinner(e) {
		...
		subAnglePoint,
		ended = false;
		helAnglePoint = Model.getPoint(Model.pointOnBoundry, -(90 - (Model.currentHelAngle/2)) * Maths.TO_RADIANS, 115);
		subAnglePoint = Model.getPoint(Model.pointOnBoundry, (90 + (Model.currentSubAngle/2)) * Maths.TO_RADIANS, 100);
		...
		_checkHit: function (point) {
			...
			if (!this.auto) {
				...
				Model.helicopter.options.forEach(function (s, i) {
					if (s.x <= point.x && s.x + s.width >= point.x && s.y + (s.height / 4) <= point.y && s.y + (s.height / 2) >= point.y) {
						status = true;
