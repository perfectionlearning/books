...
game.interaction = function(director, scene, banner){
   ...
   function drawLine(length,midPoint,ang,alpha,upDownY) {	
   	   ...
   	   for(var i=0;i<5;i++){   	   	 
			 ...
			 for(var j=0;j<5;j++){
				...
				drawPaths( game.Rays[container], {x:game.x1, y:game.y1}, {x:game.x2, y:game.y2}, {x:game.x3, y:game.y3}, false, '#d32341', 1.8, tool, false, alpha ,upDownY,true);
				container++;  
			}
		 } 
	}
    ...
    function drawMidLine(angle,decreseHeight){
		   ...
		   drawPaths( game.Rays[25], {x:midPlusX, y:midPlusY}, {x:0, y:0}, {x:xPos, y:yPos}, false, '#ff6c06', 2, shape, false, 1 ,0,false);
   ...
   function drawCirclePath(ang){   	
	...
	function moveTimer(){
		...
		centerX = initialPlusXPosition;
		objectMoveTimer = scene.createTimer(0, Number.MAX_VALUE,
				function (scene_time, timer_time, timertask_instance) {   // timeout
				},
				function (scene_time, timer_time, timertask_instance) {
						movePosition();
	...
	function drawArrowCenter(angle,decreseHeight){
		   ...
		   drawPaths( game.Rays[27], {x:midPlusX, y:midPlusY}, {x:0, y:0}, {x:xPos, y:yPos}, false, '#9b04cb', 2,shape, false, 1 ,0,false);
	}
	...
	var aActorReset = new CAAT.exemplarr().setAsButton(btnImg.getRef(), 4, 4, 5, 4, 
						function(button){							
							...
							game.scraper.x = 404.5;
							game.dragger.x = 404.5;
	...
	aActorIncDec.setLocation(410, 552);
...
//Checkbox button
    ...
    var checkBtn = new CAAT.exemplarr().setAsButton(btnImg.getRef(), 1, 0, 0, 0,
            function(button){
				isSVGShow = !isSVGShow;
                if(!isSVGShow)
                {
