...
;(function() {		
	var dashBoardEle = [];
		...
		dashBoardEle[dashBoardEle.length] = ['wellleft', 410, 0, 10];	//well left
		dashBoardEle[dashBoardEle.length] = ['wellright', 500, 0, 10];	//well right
		//dashBoard test
	var dashBoardTxt = [];
		
		dashBoardTxt[dashBoardTxt.length] = ['q =', 			'bold italic 14px Arial', 'left', 130, 30, 1, '#fff'];//Text, font_style, alignment, xPos, yPos, id
		dashBoardTxt[dashBoardTxt.length] = ['1.24 nC', 		'bold 14px Arial', 'left', 170, 31, 'qTxt', '#fff'];//Text, font_style, alignment, xPos, yPos, id
	...
	var bannerContentTxt = [];
		bannerContentTxt[bannerContentTxt.length] = ['Vary the potential difference between the two', 'capacitor plates. Observe the changes in charge.'];
		
	
	//configure buttons X, Y Position
	var buttonXYPos = [];
		buttonXYPos[buttonXYPos.length] = ['425',  '0', '35' ];//common XPosiotn, button_1 Y Position, button_2 Y Position
