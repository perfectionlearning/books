...
;(function() {		
	var dashBoardEle = [];
		...
		dashBoardEle[dashBoardEle.length] = ['well100', 600, 0, 100];	//well100
		dashBoardEle[dashBoardEle.length] = ['wellright', 700, 0, 10];	//well right
		
		//dashBoard test
	var dashBoardTxt = [];
		
		dashBoardTxt[dashBoardTxt.length] = ['Positive charge:  1.6 x 10', 	'bold 14px Arial',  'left', 364, 22, 1, '#fff'];//Text, font_style, alignment, xPos, yPos, id
		dashBoardTxt[dashBoardTxt.length] = ['Negative charge:  -1.6 x 10', 	'bold 14px Arial',  'left', 364, 55, 1, '#fff'];//Text, font_style, alignment, xPos, yPos, id
		dashBoardTxt[dashBoardTxt.length] = ['-19', 		 	'bold 10px Arial',  'left', 542,  19, 2, '#fff'];//Text, font_style, alignment, xPos, yPos, id
		dashBoardTxt[dashBoardTxt.length] = ['-19', 		 	'bold 10px Arial',  'left', 552,  52, 3, '#fff'];//Text, font_style, alignment, xPos, yPos, id
		dashBoardTxt[dashBoardTxt.length] = ['C', 			'bold 14px Arial', 'left', 565, 22, 4, '#fff'];
		dashBoardTxt[dashBoardTxt.length] = ['C', 				'bold 14px Arial', 'left', 575, 52, 5, '#fff'];
	...
	var bannerContentTxt = [];
		bannerContentTxt[bannerContentTxt.length] = ['Drag charges below to the area above to', 'view forces. Click GO to see them move.'];
		
	
	//configure buttons X, Y Position
	var buttonXYPos = [];
		buttonXYPos[buttonXYPos.length] = ['620', '505', '530', '555'];//common XPosiotn, button_1 Y Position, button_2 Y Position
	...
	{		
		...
		main.
			setLocation(0, 0);
		scene.addChild(main);
			game.__createMainImgScene(main, 0,1);	
