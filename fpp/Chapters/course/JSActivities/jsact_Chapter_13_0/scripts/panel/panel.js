...
;(function() {		
	var dashBoardEle = [];
		...
		dashBoardEle[dashBoardEle.length] = ['well100', 		624, 0, 100];
		dashBoardEle[dashBoardEle.length] = ['well100', 		652, 0, 100];		
		dashBoardEle[dashBoardEle.length] = ['wellleft', 		414, 0, 10];	
		dashBoardEle[dashBoardEle.length] = ['wellright', 		752, 0, 10];	
	...
	var bannerContentTxt = [];
		bannerContentTxt[bannerContentTxt.length] = ['Drag the masses from the right onto the area', 'above and view the gravitational forces.', 'Press GO to see them move.'];
		
	
	//configure buttons X, Y Position
	var buttonXYPos = [];
		buttonXYPos[buttonXYPos.length] = ['670',  '506', '531', '556' ];//common XPosiotn, button_1 Y Position, button_2 Y Position
