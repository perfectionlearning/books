...
var Lab = (function(){
	...
	function enterFrame(){
		...
		var pAtStart = this._points[this._wInitial];
			...
			if(pAtStart)	
			{
		this._thetaAtStart = 2 * Math.PI * this._FreqArray[this._FreqArray.length-1] *1.12* (pAtStart.x + this._phaseArray[this._phaseArray.length-1]);
			}
		if(this._first==false)
		{
		this._noOfFrames++;
		...
		for(i = waveStartArray[n]; i < waveStartArray[n-1]; i++){
			...
			if(p){
				p.y = this._AmpArray[n] * Math.sin(2 * Math.PI * this._FreqArray[n] *1.12* (p.x + this._phaseArray[n]));
		...
		for(; i < this._waveEnd; i++){
			...
			if(p){
				...
				p.y = this._AmpArray[0] * Math.sin(2 * Math.PI * this._FreqArray[0] *1.12* (p.x + this._phaseArray[0]));
