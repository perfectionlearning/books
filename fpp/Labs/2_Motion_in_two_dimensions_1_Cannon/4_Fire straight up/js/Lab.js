var Lab = (function(){
	...
	Activity.prototype.startLab = function(){
		...
		shape.graphics.clear().beginFill("#fff").rect(200, 0, 800, 800).endFill();
	...
	Activity.prototype.reset = function(){
		...
		this.layout.timeText.text = "0.00";
	...
	function createPlayers(){
		...
		var a = new Slider(this.layout.arrow1, p1Data.draggableMin, p1Data.draggableMax, stepper._min, stepper._max, this.layout.arrow1Line, {direction:"left",lineWidth:BrowserDetect.any() ? 8 : 6}, "y");
		a.reverse = true;
		...
		this.cannon.reset().addEventListener("velocityUpdated", function(e){
			scope.layout.stepper.value = e.currentTarget.velocity;
	...
	function checkBallHeight(){
		...
		if(Math.abs(this.cannon.ballHeight - Model.targetHeight) <= 0.5/*  && Math.abs(this.cannon.data("Vy")) < 0.2 */){
			if(this.cannon.data("Vy") <= 0.5 && this.cannon.data("Vy") >= -3){
		...
		this.layout.timeText.text = Model.time.toFixed(2);
		this.layout.txtAy.text = Model.g.toFixed(2);
		this.layout.txtVy.text = Math.max(-1 * this.layout.stepper.value, this.cannon.data("Vy")).toFixed(1) * 1;
	}
